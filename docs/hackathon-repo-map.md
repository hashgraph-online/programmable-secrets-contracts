...
| [`hashgraph-online/registry-broker`](https://github.com/hashgraph-online/registry-broker) | Broker-side UAID and registry integration | Used by `flow:broker` and broker-backed identity lifecycle |
| [`hashgraph-online/points-portal`](https://github.com/hashgraph-online/points-portal) | Portal integration and downstream UX | Consumes programmable-secrets contract and indexer data for portal surfaces |
...
3. Optionally run broker-backed registration and UAID issuance (`registry-broker`).
4. Query indexed data via the subgraph (`programmable-secrets-contracts/subgraph`).
5. Execute buyer/provider UX in web apps (`programmable-secrets-fe` and `points-portal`).
